...
void InvMass(Int_t save = 1)
{
  ...
  // full mass range
  c = draw1D(hInvMass[0],Form("Au+Au %s: invariant mass of di-muon pairs%s;M_{#mu#mu} (GeV/c^{2})",trigName[kTrigType],hlt_name[hlt_index]),kTRUE);
  ...
  c = draw1D(hInvMass_upsilon[0],Form("Au+Au %s: invariant mass of di-muon pairs%s;M_{#mu#mu} (GeV/c^{2})",trigName[kTrigType],hlt_name[hlt_index]),kTRUE);
  hInvMass[1]->Draw("HIST sames");
  leg->Draw();
  ...
  c = draw1D(hdiff_upsilon,Form("Au+Au %s: invariant mass of di-muon pairs%s;M_{#mu#mu} (GeV/c^{2})",trigName[kTrigType],hlt_name[hlt_index]),kFALSE,kFALSE);
  gPad->SetGridy();
  ...
  for(Int_t i=0; i<2; i++)
    {
      hInvMass_jpsi[i] = (TH1F*)hInvMass[i]->Clone(Form("%s_jpsi",hInvMass[i]->GetName()));
      hInvMass_jpsi[i]->GetXaxis()->SetRangeUser(2.5,3.5);
    }
  ...
  c = draw1D(hdiff_jpsi,Form("Au+Au %s: invariant mass of di-muon pairs%s;M_{#mu#mu} (GeV/c^{2})",trigName[kTrigType],hlt_name[hlt_index]),kFALSE);
  gPad->SetGridy();
