  ...
  for(int i=0; i<nCentBins; i++)
    {
      for(int j=0; j<6; j++)
	{
	  if(j==0) hSignal[i][j] = (TH1F*)f->Get(Form("Jpsi_BinCountYield_cent%s_weight%s",cent_Title[i],sys_name[j]));
	  else     hSignal[i][j] = (TH1F*)fSys->Get(Form("Jpsi_BinCountYield_cent%s_weight%s",cent_Title[i],sys_name[j]));
