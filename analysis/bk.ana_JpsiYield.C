...
void prod_makeYield()
{
  for(int i=0; i<4; i++)
    for(int j=0; j<7; j++)
      makeYieldRun14(i,j,1,1);
}

//===============================================
void makeYieldRun14(const int icent = 0, const int isys = 0, int savePlot = 1, int saveHisto = 1)
{
  ...
  for(Int_t i=0; i<nPtBins; i++)
    {
      hSeUL[i] = (TH1F*)fin->Get(Form("InvMass_UL_pt%s_cent%s%s",pt_Name[i],cent_Name[icent],gWeightName[gApplyWeight]));
      hSeLS[i] = (TH1F*)fin->Get(Form("InvMass_LS_pt%s_cent%s%s",pt_Name[i],cent_Name[icent],gWeightName[gApplyWeight]));
      hMixUL[i] = (TH1F*)fin->Get(Form("Mix_InvMass_UL_pt%s_cent%s%s",pt_Name[i],cent_Name[icent],gWeightName[gApplyWeight]));
      hMixLS[i] = (TH1F*)fin->Get(Form("Mix_InvMass_LS_pt%s_cent%s%s",pt_Name[i],cent_Name[icent],gWeightName[gApplyWeight]));
    }
  ...
  for(int i=0; i<nPtBins; i++)
      ...
      for(int bin=low_bin; bin<=high_bin; bin++)
	{
      ...
      int low_bin_me = hMixLS[i]->FindFixBin(scale_low+1e-4);
      int high_bin_me = hMixLS[i]->FindFixBin(scale_high-1e-4);
  ...
  for(int i=0; i<nPtBins; i++)
    {
      ...
      hMixBkg[i]->Rebin(rebin_signal*4);
      if(i>0)
	{
	  cSignal->cd(i);
	  hSeUL[i]->Draw();
  ...
  for(int i=0; i<nPtBins; i++)
      ...
      if(0)
	{
      ...
      printf("Fit combined = %1.1f +/- %1.1f -> %1.1f +/- %1.1f (%1.1fsigma, %4.2f%%)\n",bkg,bkg_err,signal,sig_err,signal/sig_err,sig_err/signal*100);
...
void makeHistos()
{
  ...
  for(Int_t j=0; j<3; j++)
    {
      if(gApplyWeight==0) sprintf(name,"m%s_%s",hName[j],trigName[kTrigType]);
      else                sprintf(name,"m%sWeight_%s",hName[j],trigName[kTrigType]);
      hnInvMass[j] = (THnSparseF*)f->Get(name);
      ...
      for(Int_t i=0; i<nPtBins; i++)
	{
	  ...
	  for(int k=0; k<nCentBins; k++)
	    {
	      hnInvMass[j]->GetAxis(5)->SetRange(centBins_low[k],centBins_high[k]);
  ...
  for(int k=0; k<nCentBins; k++)
    {
      for(Int_t i=0; i<nPtBins; i++)
	{
	  hInvMass[k][i][0]->Write(Form("InvMass_UL_pt%s_cent%s%s",pt_Name[i],cent_Name[k],gWeightName[gApplyWeight]));
	  hInvMass[k][i][1]->Write(Form("InvMass_LS_pt%s_cent%s%s",pt_Name[i],cent_Name[k],gWeightName[gApplyWeight]));
	  if(fmix)
	    {
