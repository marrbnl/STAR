...
void makeYieldRun14(const int isetup = 0, const int icent = 0, const int isys = 0, int savePlot = 0, int saveHisto = 0)
{
  ...
  cout << fileName << endl;
  cout << suffix.Data() << endl;

  TString outName = fileName;
  outName.ReplaceAll(".histo.root",".yield.root");

  TString outNameSys = outName;
  outNameSys.ReplaceAll(".yield.root",".yield.sys.root");
  ...
  for(int i=0; i<nPtBins; i++)
      ...
      if(i>-1)
	{
	  ...
	  if(i==1) hMixScale[i]->SetMinimum(0.65e-3);
	  hMixScale[i]->Draw();
  ...
  if(savePlot) 
    {
      cScaling->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sFit_LS_SE_to_ME_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
    }

  if(savePlot) 
    {
      c->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sScaleFactor_SE_to_ME_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
    }
  ...
  if(savePlot) 
    {
      cAcc->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sMix_Acceptance_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
    }
  ...
  if(savePlot) 
    {
      cSignal->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sJpsiSignal_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
    }
  ...
  for(int i=0; i<nPtBins; i++)
	...
	{
	  funcBkg[i]->SetParameter(j,funcSignal[i]->GetParameter(3+j));
	  funcBkg[i]->SetParError(j,funcSignal[i]->GetParError(3+j));
      ...
      for(int j=3; j<3+nPar; j++)
	{
      ...
      /*
      // from background fit only
      TF1 *tmpFuncBkgOnly = 0;
      if(funcForm=="pol1") tmpFuncBkgOnly = new TF1(Form("tmpFuncBkgOnly_%d",i),Polynomial1,g_sig_fit_min,g_sig_fit_max,nPar);
      if(funcForm=="pol3") tmpFuncBkgOnly = new TF1(Form("tmpFuncBkgOnly_%d",i),Polynomial3,g_sig_fit_min,g_sig_fit_max,nPar);
      for(int ip=0; ip<tmpFuncBkgOnly->GetNpar(); ip++)
      	{
      	  tmpFuncBkgOnly->SetParameter(ip,funcSignal[i]->GetParameter(ip+3));
      	}
      TH1F *hBkgOnlyFit = (TH1F*)hSignal[i]->Clone(Form("%s_fit_bkg",hSignal[i]->GetName()));
      TFitResultPtr ptr_bkg = hBkgOnlyFit->Fit(tmpFuncBkgOnly,"IR0QS");
      for(int j=0; j<nPar; j++)
      	{
      	  funcBkg[i]->SetParameter(j,tmpFuncBkgOnly->GetParameter(j));
      	  funcBkg[i]->SetParError(j,tmpFuncBkgOnly->GetParError(j));
      	}
      bkg_matrix = ptr_bkg->GetCovarianceMatrix().GetMatrixArray();
      bkg_params = ptr_bkg->GetParams();
      */
      ...
      if(i<5) 
	{
      ...
      double yield_bkg = funcBkg[i]->Integral(low_mass_tmp,high_mass_tmp) * 1./hSignal[i]->GetBinWidth(1);
      double yield_bkg_err = funcBkg[i]->IntegralError(low_mass_tmp,high_mass_tmp,bkg_params,bkg_matrix)* 1./hSignal[i]->GetBinWidth(1);
      ...
      printf("Counting method:\n");
      printf("Background = %1.1f +/- %1.1f\n",yield_bkg,yield_bkg_err);
      printf("all = %1.2f, signal = %1.2f\n",all,yield_sig);
      printf("Signal = %1.1f +/- %1.1f (%1.1fsigma, %4.2f%%)\n",yield_sig,yield_sig_err,yield_sig/yield_sig_err,yield_sig_err/yield_sig*100);
  ...
  if(savePlot) 
    {
      cFit1->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sFit_Jpsi_All_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
      cFit->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sFit_Jpsi_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
    }
  ...
  if(savePlot) 
    {
      c->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sFit_Jpsi_Mean_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
    }
  ...
  if(savePlot) 
    {
      c->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/make_JpsiYield/%s%sFit_Jpsi_Sigma_%s.pdf",run_type,run_cfg_name.Data(),sys_title[isys],suf_title.Data()));
    }
  ...
  if(saveHisto)
	...
	else if(isys==1) fout = TFile::Open(outNameSys,"update");
