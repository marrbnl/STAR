...
void getJpsiWidthEmbed(int savePlot = 0, int saveHisto = 0)
{
  ...
  for(int k=0; k<nCentBins; k++)
	...
	{
	  if(i<3) hMassVsPtEmbed[k][i] = (TH2F*)fscan->Get(Form("hRcJpsiMass_%s_final_%s",cent_Title[k],sysName_scan[i]));
	  else    hMassVsPtEmbed[k][i] = (TH2F*)fembed->Get(Form("hJpsiMassVsPt_Tpc_cent%s",cent_Title[k]));
	  for(int j=0; j<nPtBins; j++)
	    {
	      hInvMass[k][j][i] = (TH1F*)hMassVsPtEmbed[k][i]->ProjectionY(Form("SmearEmb_JpsiInvMass_cent%s_Pt%s_%s",cent_Title[k],pt_Name[j],sysName_scan[i]));
	      hInvMass[k][j][i]->Reset("AC");
	      int low_bin = hMassVsPtEmbed[k][i]->GetXaxis()->FindFixBin(ptBins_low[j]+1e-4);
	      int up_bin = hMassVsPtEmbed[k][i]->GetXaxis()->FindFixBin(ptBins_high[j]-1e-4);
	      for(int bin=low_bin; bin<=up_bin; bin++)
		{
		  TH1F *htmp = (TH1F*)hMassVsPtEmbed[k][i]->ProjectionY(Form("Projy_bin%d_%s_cent%s",bin,sysName_scan[i],cent_Title[k]),bin,bin);
		  double scale = funcInputJpsi->Integral(hMassVsPtEmbed[k][i]->GetXaxis()->GetBinLowEdge(bin),
							 hMassVsPtEmbed[k][i]->GetXaxis()->GetBinUpEdge(bin));
		  hInvMass[k][j][i]->Add(htmp, scale); 
		}
	      if(i<3) hInvMass[k][j][i]->Rebin(2);
	      hInvMass[k][j][i]->Scale(1./hInvMass[k][j][i]->Integral());
	    }
	}
    }

  TCanvas *cFit[nCentBins][nHistos];
  ...
  for(int k=0; k<nCentBins; k++)
    {
      for(int i=0; i<nHistos; i++)
	{
	  cFit[k][i] = new TCanvas(Form("FitInvMass_cent%s_%s",cent_Title[k],sysName_scan[i]),Form("FitInvMass_cent%s_%s",cent_Title[k],sysName_scan[i]), 1100, 700);
	  cFit[k][i]->Divide(3,3);
	  hInvMassMean[k][i] = new TH1F(Form("SmearEmb_JpsiMean_cent%s_%s",cent_Title[k],sysName_scan[i]), "Mean of J/psi peak;p_{T} (GeV/c)", nbins, xbins);
	  hInvMassSigma[k][i] = new TH1F(Form("SmearEmb_JpsiWidth_cent%s_%s",cent_Title[k],sysName_scan[i]), "Width of J/psi peak;p_{T} (GeV/c)", nbins, xbins);     
	  for(int j=0; j<nPtBins; j++)
	    {
	      funcInvMass[k][j][i] = new TF1(Form("func_%s",hInvMass[k][j][i]->GetName()), "gaus", 2.9, 3.4);
	      // funcInvMass[i][j] = new TF1(Form("func_%s",hInvMass[i][j]->GetName()), CrystalBall, 3, 3.2, 5);
	      // funcInvMass[i][j]->SetParameters(0, 3.096, 0.05, 10, hInvMass[i][j]->GetMaximum());
	      TH1F *hplot = (TH1F*)hInvMass[k][j][i]->Clone(Form("%s_plot",hInvMass[k][j][i]->GetName()));
	      hplot->Fit(funcInvMass[k][j][i], "IR0Q");
	      if(j==0) continue;
	      cFit[k][i]->cd(j);
	      hplot->GetXaxis()->SetRangeUser(2.9,3.4);
	      hplot->SetMarkerStyle(24);
	      hplot->SetTitle("");
	      hplot->Draw();
	      funcInvMass[k][j][i]->SetLineColor(4);
	      funcInvMass[k][j][i]->SetLineStyle(2);
	      funcInvMass[k][j][i]->Draw("sames");	  
	      hInvMassMean[k][i]->SetBinContent(j, funcInvMass[k][j][i]->GetParameter(1));
  ...
  TString legName[4] = {"Default","Lower limit", "Upper limit", "Embedding"};
  for(int i=0; i<nHistos; i++)
    {
  ...
  // Jpsi width vs. npart
  TH1F* hInvMassIntegrTmp[nPtBins_npart][nCentBins][3];
  TF1 *funcInvMassIntegrTmp[nPtBins_npart][nCentBins][3];
  TH1F *hSigmaIntegrTmp[nPtBins_npart][3];
  for(int j=0; j<nPtBins_npart; j++)
    {
      for(int i=0; i<3; i++)
	{
	  hSigmaIntegrTmp[j][i] = new TH1F(Form("SmearEmb_JpsiWidthIntegrTmp_Pt%s_%s",pt_Name_npart[j],sysName_scan[i]), "Width of J/psi peak", nCentBins-1, 0, nCentBins-1);
	  for(int k=0; k<nCentBins; k++)
	    {
	      if(k>0) hSigmaIntegrTmp[j][i]->GetXaxis()->SetBinLabel(k, Form("%s%%",cent_Name[k]));
	      hInvMassIntegrTmp[j][k][i] = (TH1F*)hMassVsPtEmbed[k][i]->ProjectionY(Form("SmearEmb_JpsiInvMass_cent%s_Pt%s_%s_tmp",cent_Title[k],pt_Name_npart[j],sysName_scan[i]));
	      hInvMassIntegrTmp[j][k][i]->Reset("AC");
	      int low_bin = hMassVsPtEmbed[k][i]->GetXaxis()->FindFixBin(ptBins_low_npart[j]+1e-4);
	      int up_bin = hMassVsPtEmbed[k][i]->GetXaxis()->FindFixBin(ptBins_high_npart[j]-1e-4);
	      for(int bin=low_bin; bin<=up_bin; bin++)
		{
		  TH1F *htmp = (TH1F*)hMassVsPtEmbed[k][i]->ProjectionY(Form("Projy3_bin%d_%s_%d_%d",bin,sysName_scan[i],j,k),bin,bin);
		  double scale = funcInputJpsi->Integral(hMassVsPtEmbed[k][i]->GetXaxis()->GetBinLowEdge(bin),
							 hMassVsPtEmbed[k][i]->GetXaxis()->GetBinUpEdge(bin));

		  double jbin = hInputJpsi->FindFixBin(hMassVsPtEmbed[k][i]->GetXaxis()->GetBinCenter(bin));
		  double scale2 = hInputJpsi->GetBinContent(jbin)/funcInputJpsi->Integral(hInputJpsi->GetXaxis()->GetBinLowEdge(jbin),
											  hInputJpsi->GetXaxis()->GetBinUpEdge(jbin));
		  hInvMassIntegrTmp[j][k][i]->Add(htmp, scale*scale2); 
		}
	      hInvMassIntegrTmp[j][k][i]->Rebin(2);
	      hInvMassIntegrTmp[j][k][i]->Scale(1./hInvMassIntegrTmp[j][k][i]->Integral());

	      TH1F *hplot = (TH1F*)hInvMassIntegrTmp[j][k][i]->Clone(Form("%s_plot",hInvMassIntegrTmp[j][k][i]->GetName()));
	      funcInvMassIntegrTmp[j][k][i] = new TF1(Form("func_%s",hInvMassIntegrTmp[j][k][i]->GetName()), "gaus", 2.95, 3.25);
  ...
  TH1F *hSigmaIntegr[nPtBins_npart][3];
  TH1F* hInvMassIntegr[nPtBins_npart][kNCent][3];
  TH1F *hMeanIntegr[nPtBins_npart][3];
  for(int j=0; j<nPtBins_npart; j++)
    {
      for(int i=0; i<3; i++)
	{
	  hSigmaIntegr[j][i] = new TH1F(Form("SmearEmb_JpsiWidthIntegr_Pt%s_%s",pt_Name_npart[j],sysName_scan[i]), "Width of J/psi peak", nCentBins_npart[j], 0, nCentBins_npart[j]);
	  hMeanIntegr[j][i] = new TH1F(Form("SmearEmb_JpsiMeanIntegr_Pt%s_%s",pt_Name_npart[j],sysName_scan[i]), "Mean of J/psi peak", nCentBins_npart[j], 0, nCentBins_npart[j]);
	  hSigmaIntegr[j][i]->SetBinContent(0, hSigmaIntegrTmp[j][i]->GetBinContent(0));
	  for(int k=0; k<kNCent-1; k++)
	    {
	      hSigmaIntegr[j][i]->GetXaxis()->SetBinLabel(k+1, Form("%s%%",cent_Name_npart[k+j*kNCent]));
	      hSigmaIntegr[j][i]->SetBinContent(k+1, hSigmaIntegrTmp[j][i]->GetBinContent(k/2+1));
	      if(j==0 && k==kNCent-2) 
		{
		  hSigmaIntegr[j][i]->GetXaxis()->SetBinLabel(k+2, Form("%s%%",cent_Name_npart[k+1]));
		  hSigmaIntegr[j][i]->SetBinContent(k+2, hSigmaIntegrTmp[j][i]->GetBinContent((k+1)/2+1));
		}
	      hInvMassIntegr[j][k][i] = (TH1F*)hInvMassIntegrTmp[j][k/2][i]->Clone(Form("SmearEmb_JpsiInvMass_cent%s_Pt%s_%s",cent_Title_npart[kNCent+k],pt_Name_npart[j],sysName_scan[i]));
	    }
	}
    }
  ...
  TH2F* hMassVsPtIntegr[kNCent-1][3];
  for(int k=0; k<kNCent-1; k++)
    {
      for(int i=0; i<3; i++)
	{
	  hMassVsPtIntegr[k][i] = (TH2F*)fscan->Get(Form("hRcJpsiMass_%s_final_%s",cent_Title_npart[kNCent+k],sysName_scan[i]));
	}
    }
  TH1F* hInvMassIntegr[nPtBins_npart][kNCent][3];
  TF1 *funcInvMassIntegr[nPtBins_npart][kNCent][3];
  TH1F *hSigmaIntegr[nPtBins_npart][3];
  TH1F *hMeanIntegr[nPtBins_npart][3];
  TCanvas *cFitIntegr[nPtBins_npart][3];
  for(int j=0; j<nPtBins_npart; j++)
    {
      for(int i=0; i<3; i++)
	{
	  cFitIntegr[j][i] = new TCanvas(Form("cFitIntegr_Pt%s_%s",pt_Name_npart[j],sysName_scan[i]),Form("cFitIntegr_Pt%s_%s",pt_Name_npart[j],sysName_scan[i]), 1100, 700);
	  cFitIntegr[j][i]->Divide(4,2);
	  hSigmaIntegr[j][i] = new TH1F(Form("SmearEmb_JpsiWidthIntegr_Pt%s_%s",pt_Name_npart[j],sysName_scan[i]), "Width of J/psi peak", nCentBins_npart[j], 0, nCentBins_npart[j]);
	  hMeanIntegr[j][i] = new TH1F(Form("SmearEmb_JpsiMeanIntegr_Pt%s_%s",pt_Name_npart[j],sysName_scan[i]), "Mean of J/psi peak", nCentBins_npart[j], 0, nCentBins_npart[j]);
	  for(int k=0; k<kNCent-1; k++)
	    {
	      hSigmaIntegr[j][i]->GetXaxis()->SetBinLabel(k+1, Form("%s%%",cent_Name_npart[k+j*kNCent]));
	      if(j==0 && k==kNCent-2) hSigmaIntegr[j][i]->GetXaxis()->SetBinLabel(k+2, Form("%s%%",cent_Name_npart[k+1]));
	      hInvMassIntegr[j][k][i] = (TH1F*)hMassVsPtIntegr[k][i]->ProjectionY(Form("SmearEmb_JpsiInvMass_cent%s_Pt%s_%s",cent_Title_npart[kNCent+k],pt_Name_npart[j],sysName_scan[i]));
	      hInvMassIntegr[j][k][i]->Reset("AC");
	      int low_bin = hMassVsPtIntegr[k][i]->GetXaxis()->FindFixBin(ptBins_low_npart[j]+1e-4);
	      int up_bin = hMassVsPtIntegr[k][i]->GetXaxis()->FindFixBin(ptBins_high_npart[j]-1e-4);
	      for(int bin=low_bin; bin<=up_bin; bin++)
		{
		  TH1F *htmp = (TH1F*)hMassVsPtIntegr[k][i]->ProjectionY(Form("Projy2_bin%d_%s_%d_%d",bin,sysName_scan[i],j,k),bin,bin);
		  double scale = funcInputJpsi->Integral(hMassVsPtIntegr[k][i]->GetXaxis()->GetBinLowEdge(bin),
							 hMassVsPtIntegr[k][i]->GetXaxis()->GetBinUpEdge(bin));
		  hInvMassIntegr[j][k][i]->Add(htmp, scale); 
		}
	      hInvMassIntegr[j][k][i]->Rebin(2);
	      hInvMassIntegr[j][k][i]->Scale(1./hInvMassIntegr[j][k][i]->Integral());

	      TH1F *hplot = (TH1F*)hInvMassIntegr[j][k][i]->Clone(Form("%s_plot",hInvMassIntegr[j][k][i]->GetName()));
	      funcInvMassIntegr[j][k][i] = new TF1(Form("func_%s",hInvMassIntegr[j][k][i]->GetName()), "gaus", 2.95, 3.25);
	      hplot->Fit(funcInvMassIntegr[j][k][i], "IR0Q");
	      hSigmaIntegr[j][i]->SetBinContent(k+1, funcInvMassIntegr[j][k][i]->GetParameter(2));
	      hSigmaIntegr[j][i]->SetBinError(k+1, funcInvMassIntegr[j][k][i]->GetParError(2));
	      hMeanIntegr[j][i]->SetBinContent(k+1, funcInvMassIntegr[j][k][i]->GetParameter(1));
	      hMeanIntegr[j][i]->SetBinError(k+1, funcInvMassIntegr[j][k][i]->GetParError(1));
	      if(j==0 && k==kNCent-2)
		{
		  hSigmaIntegr[j][i]->SetBinContent(k+2, funcInvMassIntegr[j][k][i]->GetParameter(2));
		  hSigmaIntegr[j][i]->SetBinError(k+2, funcInvMassIntegr[j][k][i]->GetParError(2));
		  hMeanIntegr[j][i]->SetBinContent(k+2, funcInvMassIntegr[j][k][i]->GetParameter(1));
		  hMeanIntegr[j][i]->SetBinError(k+2, funcInvMassIntegr[j][k][i]->GetParError(1));
		}
	      cFitIntegr[j][i]->cd(k+1);
	      hplot->SetMarkerStyle(24);
	      hplot->GetXaxis()->SetRangeUser(2.9, 3.4);
	      hplot->SetTitle("");
	      hplot->Draw();
	      TPaveText *t1 = GetTitleText(Form("ToyMC: %s%% (%s)",cent_Name_npart[kNCent+k],sysName_scan[i]),0.06);
	      t1->Draw();
	      funcInvMassIntegr[j][k][i]->SetLineColor(4);
	      funcInvMassIntegr[j][k][i]->SetLineStyle(2);
	      funcInvMassIntegr[j][k][i]->Draw("sames");	 
	    }
	  cFitIntegr[j][i]->cd(8);
	  t1 = GetPaveText(0.2,0.4,0.4,0.6,0.06,62);
	  t1->AddText(run_type);
	  t1->AddText(Form("p_{T} > %1.0f GeV/c",ptBins_low_npart[j]));
	  t1->SetTextColor(2);
	  t1->Draw();
	}
    }
  ...
  for(int j=0; j<nPtBins_npart; j++)
    {
      hSigmaIntegr[j][0]->GetXaxis()->SetLabelSize(0.05);
      list->Add(hSigmaIntegr[j][0]);
      list->Add(hSigmaIntegr[j][1]);
      list->Add(hSigmaIntegr[j][2]);
      TCanvas *c = drawHistos(list,Form("Emb_JpsiWidthVsNpart_Pt%s",pt_Name_npart[j]),Form("%s: Jpsi width extracted from simulation (p_{T} > %1.0f GeV/c)",run_type,ptBins_low_npart[j]),kFALSE,0,30,kTRUE,0,0.1,kFALSE,kTRUE,legName,kTRUE,"Toy MC",0.5,0.7,0.68,0.85,kTRUE);
      if(savePlot) c->SaveAs(Form("~/Work/STAR/analysis/Plots/%s/ana_SigExt/ToyMC_JpsiWidthVsCent_Pt%s.pdf",run_type,pt_Name_npart[j]));
      list->Clear();
    }
  */
  ...
  if(saveHisto)
	  ...
	  for(int i=0; i<3; i++)
	    {
	      for(int j=0; j<nPtBins; j++)
		{
		  hInvMass[k][j][i]->Write("",TObject::kOverwrite);
		}
	      hInvMassMean[k][i]->Write("",TObject::kOverwrite);
	      hInvMassSigma[k][i]->Write("",TObject::kOverwrite);
	    }
	}
      ...
      for(int j=0; j<nPtBins_npart; j++)
	{
	  for(int i=0; i<3; i++)
	    {
	      hSigmaIntegr[j][i]->Write("",TObject::kOverwrite);
	      hMeanIntegr[j][i]->Write("",TObject::kOverwrite);
	      for(int k=0; k<kNCent-1; k++)
		{
		  hInvMassIntegr[j][k][i]->Write("",TObject::kOverwrite);
		}
	    }
	}
    }
...
//===============================================
void Run14_signal(const int isetup = 4, const int icent = 0, const int isys = 0, int savePlot = 0, int saveHisto = 0, int saveHistoResScan = 0)
{
  // re-assign global constants
  const int nPtBins         = nPtBins_pt;
  const double* ptBins_low  = ptBins_low_pt;
  ...
  const char *sys_title[nSys] = {"","Sys.LargeScale.","Sys.SmallScale.","Sys.ScaleFit.","Sys.Rebin.","Sys.BkgFunc1.","Sys.BkgFunc2.","Sys.LargeFit.","Sys.SmallFit.","Sys.SigFunc.","Sys.FixSigDown.","Sys.FixSigUp.","Sys.LineShape"};
  ...
  TString centTitle = Form("%s",cent_Title[icent]);
  if(nCentBins==1) centTitle = "4060";
  for(int i=0; i<3; i++)
    {
      hEmbJpsiWidth[i] = (TH1F*)fscan->Get(Form("SmearEmb_JpsiWidth_cent%s_%s",centTitle.Data(),sysType[i]));
      for(int j=0; j<nPtBins; j++)
	{
	  hEmbJpsiInvMass[j][i] = (TH1F*)fscan->Get(Form("SmearEmb_JpsiInvMass_cent%s_Pt%s_%s",centTitle.Data(),pt_Name[j],sysType[i]));
	}
    }
  TH1F *hEmbJpsiMean = (TH1F*)fscan->Get(Form("SmearEmb_JpsiMean_cent%s_def",centTitle.Data()));
  ...
  // jpsi signal
  TCanvas *cSignalAll = new TCanvas(Form("InvMassAll_%s",cent_Name[icent]),Form("InvMassAll_%s",cent_Name[icent]),800,600);
  ...
  for(int i=0; i<nPtBins; i++)
      ...
      else
	{
	  ...
	  hSeLS[i]->Draw("sames HIST");
	  hMixBkg[i]->Draw("sames HIST");
	  double bin1 = hSeUL[i]->FindFixBin(3.0+1e-4);
	  double bin2 = hSeUL[i]->FindFixBin(3.2-1e-4);
	  printf("[i] %s: UL = %1.0f, LS = %1.0f, Mix = %1.0f\n",hSeUL[i]->GetName(),hSeUL[i]->Integral(bin1, bin2), hSeLS[i]->Integral(bin1, bin2), hMixBkg[i]->Integral(bin1, bin2));
	}
    }
  cSignal->cd(10);
  ...
  TF1 *funcBkg[nPtBins];
  TH1F *hChi2 = new TH1F(Form("Jpsi_Chi2_%s",suffix.Data()),"Chi2/NDF of Jpsi fit",nbins,xbins);
  ...
  for(int i=0; i<nPtBins; i++)
      ...
      if(isys==11)
	{
      ...
      double fit_yield = funcSignal[i]->GetParameter(0)/bin_width;
      double fit_yield_err = funcSignal[i]->GetParError(0)/bin_width;
      ...
      if(isys==9)
	{
      ...
      hChi2->SetBinContent(i, funcSignal[i]->GetChisquare()/funcSignal[i]->GetNDF());   
      hChi2->SetBinError(i, 1e-10);
...
void Run14_npart(const int isys = 0, int savePlot = 0, int saveHisto = 0)
{
  ...
  for(int i=0; i<nPtBins; i++)
    {
      for(int j=0; j<3; j++)
	{
	  hEmbJpsiWidth[i][j] = (TH1F*)fscan->Get(Form("SmearEmb_JpsiWidthIntegr_Pt%s_%s",pt_Name[i],sysType[j]));
	  for(int k=0; k<kNCent; k++)
	    {
	      if(k<6) hEmbJpsiInvMass[i][k][j] = (TH1F*)fscan->Get(Form("SmearEmb_JpsiInvMass_cent%s_Pt%s_%s",cent_Title[k],pt_Name[i],sysType[j]));
	      else    hEmbJpsiInvMass[i][k][j] = (TH1F*)fscan->Get(Form("SmearEmb_JpsiInvMass_cent6080_Pt%s_%s",pt_Name[i],sysType[j]));
	      hEmbJpsiInvMass[i][k][j]->SetName(Form("SmearEmb_JpsiInvMass_cent%s_Pt%s_%s",cent_Title[k],pt_Name[i],sysType[j]));
	    }
	}
    }
      ...
      for(int t=0; t<nSetup; t++)
	{
	  ...
	  for(int k=0; k<nCentBins[i]; k++)
	      ...
	      for(int bin=low_bin_me; bin<=high_bin_me; bin++)
		{
	      ...
	      TPaveText *text = GetTitleText(Form("%s (%s%%)",pt_Title_npart[i],cent_Name[i*kNCent+k]),0.05);
	      text->Draw();
	  ...
	  for(int k=0; k<nCentBins[i]; k++)
	    {
	      ...
	      // scale mix event background
	      int low_bin = hMixBkg[i][k][t]->FindFixBin(2.3);
	      int up_bin  = hMixBkg[i][k][t]->FindFixBin(4.2);
	      for(int ibin=low_bin; ibin<=up_bin; ibin++)
	      	{
	      	  double mass = hMixBkg[i][k][t]->GetBinCenter(ibin);
	      	  double scale = funcScale[i][k][t]->Eval(mass);
	      	  hMixBkg[i][k][t]->SetBinContent(ibin, scale * hMixBkg[i][k][t]->GetBinContent(ibin));
	      ...
	      if(i==1 && k==nCentBins[i]-1) 
		{
	      ...
	      hSeLS[i][k][t]->Rebin(g_bin_width_tmp/hSeLS[i][k][t]->GetBinWidth(1));
	      hMixBkg[i][k][t]->SetLineColor(4);
	      hMixBkg[i][k][t]->Rebin(g_bin_width_tmp/hMixBkg[i][k][t]->GetBinWidth(1));
  ...
  for(int i=0; i<nPtBins; i++)
      ...
      for(int t=0; t<nSetup; t++)
	{
	  ...
	  for(int k=0; k<nCentBins[i]; k++)
	      ...
	      if(t>0)
		{
	      ...
	      if(isys==12) bin_width /= hSignalTemplate->GetBinWidth(1);
	      double fit_yield = funcSignal[i][k][t]->GetParameter(0)/bin_width;
	      double fit_yield_err = funcSignal[i][k][t]->GetParError(0)/bin_width;
	      ...
	      for(int j=nSigPar; j<nSigPar+nPar; j++)
		{
	      ...
	      for(int par=0; par<3; par++)
		{
		  funcJpsi_temp->SetParameter(par, funcSignal[i][k][t]->GetParameter(par));
		  funcJpsi_temp->SetParError(par, funcSignal[i][k][t]->GetParError(par));
		}
	      double efficiency = funcJpsi_temp->Integral(low_bin_mass,high_bin_mass)/funcJpsi_temp->GetParameter(0);
	      double yield_sig = (yield_all - yield_bkg)/efficiency;
	      double yield_sig_err = TMath::Sqrt(yield_all_err*yield_all_err+yield_bkg_err*yield_bkg_err)/efficiency;
      ...
      c = draw1D(hMean[i][0],Form("Mean of Gaussian fit to Jpsi signal (%s);;mean",pt_Title_npart[i]));
      ...
      if(savePlot && isys==0) 
	{
      ...
      c = draw1D(hSigma[i][0],Form("Sigma of Gaussian fit to Jpsi signal (%s);;#sigma",pt_Title_npart[i]));
...
void studyLowStat(int savePlot = 1)
{
 ...
 // open input and output files
  ...
  TH1F *hSeLS[nCentBins];
  TH1F *hMixUL[nCentBins];
  TH1F *hMixLS[nCentBins];
  TH1F *hMixBkg[nCentBins];
  const int ptBin[nCentBins] = {9, 9, 8, 8, 6};
  for(Int_t icent=0; icent<nCentBins; icent++)
    {
      hSeUL[icent] = (TH1F*)fin->Get(Form("InvMass_UL_pt%s_cent%s_weight",pt_Name[ptBin[icent]],cent_Name[icent]));
      hSeLS[icent] = (TH1F*)fin->Get(Form("InvMass_LS_pt%s_cent%s_weight",pt_Name[ptBin[icent]],cent_Name[icent]));
      hMixUL[icent] = (TH1F*)fin->Get(Form("Mix_InvMass_UL_pt%s_cent%s",pt_Name[ptBin[icent]],cent_Name[icent]));
      hMixLS[icent] = (TH1F*)fin->Get(Form("Mix_InvMass_LS_pt%s_cent%s",pt_Name[ptBin[icent]],cent_Name[icent]));
    }
  ...
  for(Int_t icent=0; icent<nCentBins; icent++)
    {
      hEmbJpsiWidth[icent] = (TH1F*)fscan->Get(Form("SmearEmb_JpsiWidth_cent%s_def",cent_Title[icent]));
    }

  // mixed event scaling
  TH1F *hMixScale[nCentBins];
  TF1 *funcScale[nCentBins];
  TCanvas *cScaling = new TCanvas("ME_scale","ME_scale",1100,650);
  cScaling->Divide(3, 2);
  for(Int_t icent=0; icent<nCentBins; icent++)
    {
      hMixScale[icent] = (TH1F*)hSeLS[icent]->Clone(Form("LS_ratio_SE_to_ME_pt%s_cent%s",pt_Name[ptBin[icent]],cent_Name[icent]));
      hMixScale[icent]->Rebin(5);
      TH1F *htmp = (TH1F*)hMixLS[icent]->Clone(Form("%s_clone",hMixLS[icent]->GetName()));
      htmp->Rebin(int(hMixScale[icent]->GetBinWidth(1)/hMixLS[icent]->GetBinWidth(1)));
      hMixScale[icent]->Divide(htmp);

      // fitting method
      funcScale[icent] = new TF1(Form("Fit_%s",hMixScale[icent]->GetName()),"pol1",2.7,3.8);
      hMixScale[icent]->Fit(funcScale[icent],"IR0Q");
      
      // plotting
      cScaling->cd(icent+1);
      hMixScale[icent]->SetTitle(";M_{#mu#mu} [GeV/c^{2}];LS: SE/ME");
      hMixScale[icent]->SetMarkerStyle(21);
      hMixScale[icent]->GetXaxis()->SetRangeUser(2.5,4);
      hMixScale[icent]->SetMaximum(2*hMixScale[icent]->GetMaximum());
      hMixScale[icent]->Draw();
      funcScale[icent]->SetLineColor(2);
      funcScale[icent]->Draw("sames");
      TPaveText *t = GetTitleText(Form("%s (%s%%)",pt_Title_pt[ptBin[icent]],cent_Name[icent]),0.06);
      t->Draw();
    }
  ...
  for(Int_t icent=0; icent<nCentBins; icent++)
    {
      ...
      // scale mix event background
      hMixBkg[icent] = (TH1F*)hMixUL[icent]->Clone(Form("mix_bkg_pt%s_cent%s",pt_Name[ptBin[icent]],cent_Name[icent]));
      int low_bin = hMixBkg[icent]->FindFixBin(2.3);
      int up_bin  = hMixBkg[icent]->FindFixBin(4.2);
      for(int ibin=low_bin; ibin<=up_bin; ibin++)
	{
	  double mass = hMixBkg[icent]->GetBinCenter(ibin);
	  double scale = funcScale[icent]->Eval(mass);
	  hMixBkg[icent]->SetBinContent(ibin, scale * hMixBkg[icent]->GetBinContent(ibin));
	  hMixBkg[icent]->SetBinError(ibin, scale * hMixBkg[icent]->GetBinError(ibin));
	}
      
      // plot invariant mass
      cSignal->cd(1);
      double g_bin_width_tmp = 0.1;
      hSeUL[icent]->Rebin(g_bin_width_tmp/hSeUL[icent]->GetBinWidth(1));
      hSeUL[icent]->SetTitle("");
      hSeUL[icent]->SetMarkerStyle(21);
      hSeUL[icent]->SetMarkerColor(2);
      hSeUL[icent]->SetLineColor(2);
      hSeUL[icent]->GetXaxis()->SetRangeUser(2.5,4);
      hSeLS[icent]->Rebin(g_bin_width_tmp/hSeLS[icent]->GetBinWidth(1));
      hMixBkg[icent]->SetLineColor(4);
      hMixBkg[icent]->Rebin(g_bin_width_tmp/hMixBkg[icent]->GetBinWidth(1));
      hSeUL[icent]->Draw();
      hSeLS[icent]->Draw("sames HIST");
      hMixBkg[icent]->Draw("sames HIST");
      TPaveText *t = GetTitleText(Form("%s%%: %s",cent_Name[icent],pt_Title_pt[ptBin[icent]]),0.06);
      t->Draw();
      leg = new TLegend(0.55,0.65,0.75,0.85);
      leg->SetBorderSize(0);
      leg->SetFillColor(0);
      leg->SetTextSize(0.035);
      leg->AddEntry(hSeUL[icent],"Unlike sign","P");
      leg->AddEntry(hSeLS[icent],"Like sign (++)+(--)","L");
      leg->AddEntry(hMixBkg[icent],"Mix UL","L");
      leg->Draw();

      // fit UL
      cSignal->cd(2);
      TH1F *hFit = (TH1F*)hSeUL[icent]->Clone(Form("%s_fit",hSeUL[icent]->GetName()));
      hFit->SetMarkerColor(1);
      hFit->SetLineColor(1);
      hFit->Draw();
      for(int j=0; j<2; j++)
	{
	  TF1 *func = new TF1(Form("%s_func%d",hSeUL[icent]->GetName(),j), "gausn(0)+pol1(3)", 2.5, 4.0);
	  func->SetParameters(1, 3.09, 0.05, 1);
	  if(j==0) hFit->Fit(func, "IR0Q");
	  else     hFit->Fit(func, "IR0QL");
      ...
      // fit UL - ME
      cSignal->cd(3);
      TH1F *hFit2 = (TH1F*)hSeUL[icent]->Clone(Form("%s_fit2",hSeUL[icent]->GetName()));
      for(int bin=1; bin<=hFit2->GetNbinsX(); bin++)
	{
	  if(hFit2->GetBinContent(bin)==0)
	    {
	      hFit2->SetBinContent(bin,0);
	      hFit2->SetBinError(bin,1.4);
	    }
	}
      hFit2->Add(hMixBkg[icent],-1);
