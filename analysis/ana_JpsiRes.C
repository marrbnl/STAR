...
void flatPt(const int icent, const bool savePlot)
{
  ...
  TString name[4] = {"MCinput","TPCreco","MTDreco","TPCemb"};
  ...
  for(int i=0; i<3; i++)
    {
      ...
      hRcJpsiMass[i] = new TH2F(Form("hRcJpsiMass_%s_%s",name[i+1].Data(),cent_Title[icent]),Form("Mass distribution of reconstructed J/#psi (%s%%);mass (GeV/c^{2})",cent_Name[icent]),110,0,11,1400,0,14);
    }

  smear(jpsiMass, icent, 1e7, 0, 0, hJpsiPt[0], hRcJpsiPt[2], hRcJpsiMass[2], hRcJpsiPt[0], hRcJpsiMass[0], hRcJpsiPt[1], hRcJpsiMass[1], 0, 0, 0, 0, 0, 0, 0);
  ...
  for(int i=0; i<2; i++)
      ...
      if(savePlot)
	{
      ...
      TH1F *htmp = (TH1F*)hRcJpsiMass[i]->ProjectionY(Form("%s_proy",hRcJpsiMass[i]->GetName()));
      htmp->Sumw2();
      htmp->Scale(1./htmp->Integral());
      htmp->SetLineColor(2);
      htmp->Draw("sames HIST");
      if(savePlot)
	{
      ...
      htmp = (TH1F*)hJpsiMass[i+1]->Clone(Form("%s_linear",hJpsiMass[i+1]->GetName()));
      ...
      if(savePlot)
	{
      ...
      TH1F *hMassRatio = (TH1F*)htmp->Clone(Form("hMassRatio_%s_%s",name[i+1].Data(),cent_Title[icent]));
