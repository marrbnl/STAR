  ...
  */
  ...
  for(int j=0; j<2; j++)
      ...
      for(int i=0; i<nCentBins; i++)
	{
      ...
      c = drawHistos(list,Form("%s_Signif",name[j]),Form("%s: J/psi significance;p_{T} (GeV/c)",name[j]),kFALSE,0,30,kTRUE,1e-6,15,kFALSE,kTRUE,legName,kTRUE,"Centrality",0.15,0.3,0.6,0.85,kTRUE);
      if(savePlot) 
	{
