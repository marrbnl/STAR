...
void qa_vertex(const Int_t save = 1)
{
  ...
  for(Int_t i=0; i<3; i++)
    {
      hDvzVsTpc[i] = (TH2F*)f->Get(Form("hDvzVsTpc_%s",trigName[i]));
      ...
      line->Draw();
      if(save) c->SaveAs(Form("~/Work/STAR/analysis/Plots/qa_vertex/%s.DvzVsTpcVz_%s.png",cut,trigName[i]));
    }
  ...
  for(Int_t i=0; i<3; i++)
    {
      ...
      c = draw2D(hVxVyTpc[i]);
      if(save) c->SaveAs(Form("~/Work/STAR/analysis/Plots/qa_vertex/%s.VxVyTpcWithCut_%s.png",cut,trigName[i]));
  ...
  if(save) c->SaveAs(Form("~/Work/STAR/analysis/Plots/qa_vertex/%s.VrTpcWithCut.png",cut));
  ...
  for(Int_t i=0; i<3; i++)
    {
      ...
      if(save) c->SaveAs(Form("~/Work/STAR/analysis/Plots/qa_vertex/%s.VzTpc_CompareCut_%s.png",cut,trigName[i]));
    }
}
