...
void daughters(Int_t save = 1)
{
  ...
  for(Int_t i=0; i<3; i++)
    {
      ...
      list->Add(hDiff[i]);
      legName[i] = Form("p_{T,1}>%1.1f, p_{T,2}>%1.1f",pt1_cut[i],pt2_cut[i]);
      ...
      c = draw1D(hInvMass[0][i],Form("Au+Au %s: invariant mass of di-muon pairs%s",trigName[kTrigType],hlt_name[hlt_index]),kFALSE);
      hInvMass[1][i]->Draw("HIST sames");
      TLegend *leg = new TLegend(0.6,0.65,0.75,0.85);
      leg->SetBorderSize(0);
      leg->SetFillColor(0);
      leg->SetTextSize(0.04);
      leg->SetHeader(Form("p_{T,1}>%1.1f, p_{T,2}>%1.1f",pt1_cut[i],pt2_cut[i]));
  ...
  c = drawHistos(list,"InvMass_ComparePt1Pt2Cuts",Form("Au+Au %s: invariant mass of di-muon pairs%s (US-LS)",trigName[kTrigType],hlt_name[hlt_index]),kTRUE,3.0,3.2,kTRUE,-200,300,kFALSE,kTRUE,legName,kTRUE,"",0.15,0.25,0.7,0.88);
  gPad->SetGridy();
...
void InvMass(Int_t save = 1)
{
  ...
  THnSparseF *hnInvMass[3];
  TH1F *hInvMass[3];
  Double_t pt1_cut = 2, pt2_cut = 1.2;
  for(Int_t j=0; j<3; j++)
    {
      hnInvMass[j] = (THnSparseF*)f->Get(Form("%s_%s",hName[j],trigName[kTrigType]));
  ...
  hDiff->GetXaxis()->SetRangeUser(2.5,3.5);
  c = draw1D(hDiff,Form("Au+Au %s: invariant mass of di-muon pairs%s",trigName[kTrigType],hlt_name[hlt_index]));
  gPad->SetGridy();
  //TLine *line = GetLine(3.097,0,3.097,hDiff->GetMaximum()*0.8,1);
  //line->Draw();
  if(save) c->SaveAs(Form("~/Work/STAR/analysis/Plots/InvMass/%s.InvMass_US-LS_pt1_%1.1f_pt2_%1.1f.png",run_config,pt1_cut,pt2_cut));
  ...
  TLegend *leg = new TLegend(0.4,0.75,0.5,0.88);
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
  leg->SetTextSize(0.04);
  ...
  leg->Draw();
  if(save) c->SaveAs(Form("~/Work/STAR/analysis/Plots/InvMass/%s.InvMassDis_pt1_%1.1f_pt2_%1.1f.png",run_config,pt1_cut,pt2_cut));
}
