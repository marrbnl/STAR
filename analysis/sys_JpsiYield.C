 ...
 // re-assign global constants
  ...
  for(int i=0; i<nCentBins; i++)
      ...
      for(int j=0; j<nSys_used; j++)
	{
      ...
      if(i==0)
	{
	  double xmin = 0, xmax = 0.45, ymin = 0.6, ymax = 0.88;
	  TLegend *leg[2];
	  for(int l=0; l<2; l++)
	    {
      ...
      for(int bin=1; bin<=hSys[i]->GetNbinsX(); bin++)
	{
	  ...
	  //if(i==0 && bin==9) sys = max[i][bin-2];
	  //if(i==1 && bin>=8) sys = max[i][6];
	  if(bin>=7) sys = max[i][5];
  ...
  leg[1]->Draw();
  c->cd(2);
  TLegend *leg2 =  new TLegend(0.2, 0.75, 0.4, 0.85);
  leg2->SetBorderSize(0);
  leg2->SetFillColor(0);
  leg2->SetTextSize(0.045);
  leg2->SetHeader(run_type);
  leg2->AddEntry(hSys[0],"Maximum deviation","L");
...
void YieldVsNpart(int savePlot = 0, int saveHisto = 0)
{
 // re-assign global constants
  ...
  // for(int i=0; i<nPtBins; i++)
  //   {
  //     TCanvas *c = new TCanvas(Form("Sys_Chi2_pt%s",pt_Name[i]),Form("Sys_Chi2_pt%s",pt_Name[i]),800,600);
  //     for(int j=0; j<nSys_used; j++)
  // 	{
  // 	  if(j==0) hChi2[i][j] = (TH1F*)fdata->Get(Form("Jpsi_FitChi2_pt%s_weight%s",pt_Name[i],sys_name[j]));
  // 	  else hChi2[i][j] = (TH1F*)fsys->Get(Form("Jpsi_FitChi2_pt%s_weight%s",pt_Name[i],sys_name[j]));
  // 	  hChi2[i][j]->SetMarkerSize(1);
  // 	  hChi2[i][j]->SetMarkerStyle(20+j);
  // 	  hChi2[i][j]->SetMarkerColor(color_sys[j]);
  // 	  if(j==0) hChi2[i][j]->Draw();
  // 	  else     hChi2[i][j]->Draw("samesP");
  // 	}
  //   } 
  ...
  for(int i=0; i<nPtBins; i++)
      ...
      for(int j=0; j<nSys_used; j++)
	{
	  ...
	  else hSignal[i][j] = (TH1F*)fsys->Get(Form("Jpsi_%sYield_pt%s_weight%s",method,pt_Name[i],sys_name[j]));
	  hSignal[i][j]->SetMarkerSize(1);
	  hSignal[i][j]->SetMarkerStyle(20+j);
	  hSignal[i][j]->SetMarkerColor(color_sys[j]);
